						...
						loff_t *f_pos)
{
	...
	if (strncmp(buf, wlan_off_str, strlen(wlan_off_str)) == 0) {
		if (monitor_active &&
		    !uid_eq(current_euid(), GLOBAL_ROOT_UID)) {
			pr_warn_ratelimited("Ignoring framework wifi OFF while monitor mode is active (%s)\n",
					     current->comm);
			goto exit;
		}
	...
	if (strncmp(buf, wlan_on_str, strlen(wlan_on_str)) == 0) {
		if (monitor_active &&
		    !uid_eq(current_euid(), GLOBAL_ROOT_UID)) {
			pr_warn_ratelimited("Ignoring framework wifi ON while monitor mode is active (%s)\n",
					     current->comm);
			goto exit;
		}
